...
function ODT:POI_CreateFramePools()
    ODT.poi_framePools = ODT.poi_framePools or CreateFramePoolCollection()
    ODT.poi_framePools:CreatePool("Button", ODT.main_frame.mapPanelFrame, "MapLinkPinTemplate")
    ODT.poi_framePools:CreatePool("Button", ODT.main_frame.mapPanelFrame, "DeathReleasePinTemplate")
    ODT.poi_framePools:CreatePool("Button", ODT.main_frame.mapPanelFrame, "VignettePinTemplate")
    ODT.poi_framePools:CreatePool("Frame", ODT.main_frame.mapPanelFrame, "ODTAnimatedLineTemplate")
end
...
local function POI_SetDevOptions(frame,poi)
    ...
    frame:SetScript("OnMouseDown", function(self, button)
        ...
        if button == "RightButton" then
            local pois = ODT.mapPOIs[db.currentDungeonIdx][ODT:GetCurrentSubLevel()]
...
local function POI_SetOptions(frame,type,poi,homeSublevel)
    ...
    if type == "mapLink" then
        ...
        frame:SetScript("OnClick",function()
            ODT:SetCurrentSubLevel(poi.target)
            ODT:UpdateMap()
            ODT:ZoomMapToDefault()
        end)
        frame:SetScript("OnEnter",function()
            GameTooltip:SetOwner(UIParent, "ANCHOR_CURSOR")
            GameTooltip:AddLine(ODT:GetDungeonSublevels()[db.currentDungeonIdx][poi.target], 1, 1, 1, 1)
    ...
    if type == "nyalothaSpire" then
        ...
        frame:SetScript("OnMouseUp",function(self,button)
            if button == "RightButton" then
                --reset npc location
                ODT:GetRiftOffsets()[self.npcId]=nil
                ODT:UpdateMap()
                if ODT.liveSessionActive and ODT:GetCurrentPreset().uid == ODT.livePresetUID then
                    ODT:LiveSession_SendCorruptedPositions(ODT:GetCurrentPreset().value.riftOffsets)
                end
            end
            if button == "LeftButton" then
                local _, connections = ODT:FindConnectedDoor(frame.npcId,1)
                if connections then
                    ODT:SetCurrentSubLevel(connections[#connections].target)
                    ODT:UpdateMap()
                end
            end
        end)
        ...
        frame:SetScript("OnEnter",function()
            ...
            local blips = ODT:GetDungeonEnemyBlips()
                ...
                for _,blip in pairs(blips) do
                    if blip.data.id == poi.npcId then
                        ...
                        if isBlipSameWeek then
                            ...
                            frame.animatedLine = ODT:ShowAnimatedLine(ODT.main_frame.mapPanelFrame,frame,blipFrame,nil,nil,nil,nil,nil,not frame.isSpire,frame.animatedLine)
            ...
            local connectedDoor, connections = ODT:FindConnectedDoor(frame.npcId,1)
            if connectedDoor then
                if frame.isSpire then
                    frame.animatedLine = ODT:ShowAnimatedLine(ODT.main_frame.mapPanelFrame,frame,connectedDoor,nil,nil,nil,nil,nil,not frame.isSpire,frame.animatedLine)
                end
                local sublevelName = ODT:GetSublevelName(nil,connections[#connections].target)
                local npcName = ODT:GetNPCNameById(frame.npcId)
        ...
        frame:SetScript("OnLeave",function()
            ...
            if frame.isSpire then
                ODT:HideAnimatedLine(frame.animatedLine)
            end
        end)
        --check expanded status
        if ODT:IsNPCInPulls(poi) then
...
function ODT:POI_HideAllPoints()
...
---POI_UpdateAll
function ODT:POI_UpdateAll()
    twipe(points)
    db = ODT:GetDB()
    local framePools = ODT.poi_framePools
    ...
    if not ODT.mapPOIs[db.currentDungeonIdx] then return end
    local currentSublevel = ODT:GetCurrentSubLevel()
    local pois = ODT.mapPOIs[db.currentDungeonIdx][currentSublevel]
    if not pois then return end
    local preset = ODT:GetCurrentPreset()
    local teeming = ODT:IsPresetTeeming(preset)
    local scale = ODT:GetScale()
    local week = ODT:GetEffectivePresetWeek(preset)
    for poiIdx,poi in pairs(pois) do
        ...
        then
            ...
            poiFrame:SetPoint("CENTER", ODT.main_frame.mapPanelTile1,"TOPLEFT",poi.x*scale,poi.y*scale)
...
local function animateLine(self, elapsed)
    ...
    for i=1,numLines + 1 do
        ...
        tex:SetPoint("TOPLEFT", ODT.main_frame.mapPanelTile1,"TOPLEFT",tX - (self.sizeX / 2),tY - (self.sizeY / 2))
        tex:SetPoint("BOTTOMRIGHT", ODT.main_frame.mapPanelTile1,"TOPLEFT",tX + (self.sizeX / 2),tY + (self.sizeY / 2))
...
local function createAnimatedLine(parent)
    local animatedLine = ODT.poi_framePools:Acquire("ODTAnimatedLineTemplate")
    animatedLine:Show()
...
function ODT:ShowAnimatedLine(parent, frame1, frame2, sizeX, sizeY, gap, color, speed, selected, animatedLine)
    if not (frame1 and frame2 and (not frame1:IsForbidden()) and (not frame1:IsForbidden())) then
        return nil
    end
    texturePool = texturePool or CreateTexturePool(ODT.main_frame.mapPanelFrame ,"OVERLAY",7,nil)
    ...
    local scale = ODT:GetScale()
    local mapSizex,mapSizey = ODT:GetDefaultMapPanelSize()
...
function ODT:KillAllAnimatedLines()
    local linePool = self.poi_framePools:GetPool("ODTAnimatedLineTemplate")
...
function ODT:DrawAllAnimatedLines()
    local week = self:GetEffectivePresetWeek()
    for _,blip in pairs(ODT:GetDungeonEnemyBlips()) do
        if not blip:IsShown() and blip.data.corrupted then
            ODT:HideAnimatedLine(blip.animatedLine)
        elseif blip.data.corrupted and blip.selected then
            ...
            local connectedDoor = ODT:FindConnectedDoor(blip.data.id)
            connectedFrame = connectedDoor or connectedFrame
            blip.animatedLine = ODT:ShowAnimatedLine(ODT.main_frame.mapPanelFrame,connectedFrame,blip,nil,nil,nil,nil,nil,blip.selected)
    ...
    local _,activeSpires = ODT.poi_framePools:GetPool("VignettePinTemplate"):EnumerateActive()
    for poiFrame,_ in pairs(activeSpires) do
        if poiFrame.spireIndex and poiFrame.npcId and not poiFrame.isSpire and not poiFrame.animatedLine then
            local connectedDoor = ODT:FindConnectedDoor(poiFrame.npcId,1)
            if connectedDoor then
                poiFrame.animatedLine = ODT:ShowAnimatedLine(ODT.main_frame.mapPanelFrame,poiFrame,connectedDoor,nil,nil,nil,nil,nil,not poiFrame.isSpire)
            end
        end
    end
end

function ODT:HideAnimatedLine(animatedLine)
...
function ODT:FindConnectedDoor(npcId, numConnection)
